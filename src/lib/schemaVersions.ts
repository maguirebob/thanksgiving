...
export const SCHEMA_VERSIONS: Record<string, SchemaVersion> = {
  ...
  '3.1.12': {
    ...
    optionalColumns: {},
    migrationStatus: 'complete',
    notes: 'Email service migration: Successfully migrated from Mailgun to Resend API. Updated authentication from Basic Auth to Bearer token, simplified environment variables, and achieved 43-53% cost savings. Email functionality tested and working.'
  },
  '3.1.13': {
    version: '3.1.13',
    requiredTables: [
    ...
    optionalColumns: {},
    migrationStatus: 'complete',
    notes: 'Blog image upload fix: Resolved multer "Unexpected field" error by updating configuration to handle both single (blog_image) and multiple (blog_images) file uploads. Simplified image processing logic to seamlessly support 1, 2, or multiple images.'
  },
  '3.1.14': {
    version: '3.1.14',
    requiredTables: [
    ...
    optionalColumns: {},
    migrationStatus: 'complete',
    notes: 'Scrapbook blog layout fix: Fixed blog page generation to display titles and content at the top of pages instead of alongside images. Each blog now gets separate pages with proper image distribution (max 2 per page). Added journal page CSS styling with flexbox layout for vertical content arrangement.'
  },
  '3.1.15': {
    version: '3.1.15',
    requiredTables: [
    ...
    optionalColumns: {},
    migrationStatus: 'complete',
    notes: 'Build process fix: Updated build script to copy template files from src/templates/ to dist/templates/ during compilation. Resolves scrapbook generation error in deployed environments where template files were not available in the compiled JavaScript output.'
  },
  '3.1.16': {
    version: '3.1.16',
    requiredTables: [
    ...
    optionalColumns: {},
    migrationStatus: 'complete',
    notes: 'Scrapbook error handling improvements: Added comprehensive error handling for scrapbook generation with detailed debugging information. Improved template path resolution to handle multiple possible locations in deployed environments. Added validation for year parameters and better error messages for different failure scenarios.'
  },
  '3.1.17': {
    version: '3.1.17',
    requiredTables: [
    ...
    optionalColumns: {},
    migrationStatus: 'complete',
    notes: 'Scrapbook debugging improvements: Added detailed constructor logging to identify template path resolution issues. Enhanced debugging output to show __dirname, process.cwd(), and each template path being tested during ScrapbookHtmlGenerator initialization.'
  },
  '3.1.18': {
    version: '3.1.18',
    requiredTables: [
    ...
    optionalColumns: {},
    migrationStatus: 'complete',
    notes: 'Scrapbook route debugging: Added detailed logging to scrapbook route handler to identify where the 500 error occurs before ScrapbookHtmlGenerator constructor runs. Enhanced debugging to show request parameters and instance creation process.'
  },
  '3.1.19': {
    version: '3.1.19',
    requiredTables: [
  ...
  '3.1.21': {
    ...
    optionalColumns: {},
    migrationStatus: 'complete',
    notes: 'Scrapbook debugging cleanup and front/back cover styling fix: Removed all debugging console.log statements from ScrapbookHtmlGenerator and scrapbookRoutes.ts. Fixed front and back cover title styling to match simple.html with dark embossed text instead of gold. Updated test to use realistic API endpoints instead of direct imports.'
  },
  '3.1.23': {
    version: '3.1.23',
    requiredTables: [
