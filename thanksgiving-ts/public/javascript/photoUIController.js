...
class PhotoUIController {
   ...
   */
  initializeUI() {
    console.log('PhotoUIController: Initializing UI...');
  ...
  displayPhotos(photos) {
    console.log('PhotoUIController: displayPhotos called with', photos.length, 'photos');
    console.log('PhotoUIController: UI elements available:', {
      photosGrid: !!this.photosGrid,
      noPhotosMessage: !!this.noPhotosMessage
    });
    
    if (!this.photosGrid || !this.noPhotosMessage) {
      console.error('PhotoUIController: Required UI elements not found!');
      return;
    }

    if (photos.length === 0) {
      console.log('PhotoUIController: No photos to display, checking if we should clear existing photos');
      // Only clear if there are no photos AND we don't already have photos displayed
      const currentPhotoCount = this.photosGrid.children.length;
      if (currentPhotoCount === 0) {
        console.log('PhotoUIController: No existing photos, showing no photos message');
        this.photosGrid.innerHTML = '';
        this.noPhotosMessage.style.display = 'block';
      } else {
        console.log('PhotoUIController: Keeping existing photos, not clearing grid');
      }
      return;
    }

    console.log('PhotoUIController: Displaying', photos.length, 'photos');
    this.noPhotosMessage.style.display = 'none';
    this.photosGrid.innerHTML = '';

    photos.forEach((photo, index) => {
      console.log(`PhotoUIController: Creating card for photo ${index + 1}:`, photo);
      const photoCard = this.createPhotoCard(photo);
      console.log(`PhotoUIController: Appending photo card ${index + 1} to grid`);
      this.photosGrid.appendChild(photoCard);
      console.log(`PhotoUIController: Photo card ${index + 1} appended successfully`);
    });
    
    console.log('PhotoUIController: Photos displayed successfully');
    console.log('PhotoUIController: Final photosGrid children count:', this.photosGrid.children.length);
  }
  ...
  createPhotoCard(photo) {
    console.log('PhotoUIController: Creating photo card for photo:', photo);
    
    const photoDiv = document.createElement('div');
    photoDiv.className = 'col-md-4 col-lg-3 mb-4';

    // Use PhotoApiClient to get authenticated photo URL
    const imageUrl = this.photoManager.apiClient.getPhotoUrl(photo.id);
    console.log('PhotoUIController: Generated image URL:', imageUrl);
    ...
    // Add image loading debugging
    img.onload = () => console.log('PhotoUIController: Image loaded successfully:', imageUrl);
    img.onerror = (error) => console.error('PhotoUIController: Image failed to load:', imageUrl, error);
    console.log('PhotoUIController: Setting image src to:', imageUrl);
    ...
    console.log('PhotoUIController: Photo card created successfully:', photoDiv);
    return photoDiv;
  }
